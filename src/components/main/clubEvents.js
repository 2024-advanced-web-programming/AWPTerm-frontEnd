...
const ClubEvents = () => {
  ...
  const [noticeType, setNoticeType] = useState("all"); // 콤보박스 상태
  ...
  const getPost = () => {
    ...
    if(noticeType === 'club') {
      url = process.env.REACT_APP_SERVER_URL + "/board/inquiry/noticeType";
    }
    else if(noticeType === 'all') {
      url = process.env.REACT_APP_SERVER_URL + "/board/all/전체_공지";
    }
